...
store = LocalFileStore("C:/Users/burha/Mentorskool/Self Learning/GenAI/RAG/cache")

# instantiate a loader. If there are multiple pdf, and we have to load them together then use PyPDFDirectoryLoader
loader = PyPDFLoader("C:/Users/burha/Mentorskool/Self Learning/GenAI/RAG/Enqurious_ETL_DB document.pdf")
...
embedder = CacheBackedEmbeddings.from_bytes_store(embedding_model, store)


vector_db = FAISS.from_documents(pages, embedder)
...
qa_chain = RetrievalQA.from_chain_type(
    llm=llm, chain_type="stuff", return_source_documents=True, retriever=retriever
)
...
tools = [
    ...
    Tool.from_function(
        ...
        description="When user asks the question where we don't get results from database then access this tool to answer the query.",
        func=qa_chain.run,
    ),
]
