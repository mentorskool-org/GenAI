...
tools = [
    ...
    Tool.from_function(
        name="Local DB",
        func=db_chain.run,
        description="Useful when we need to answer the questions regarding learners, clients, progress, skills, scores, etc. Also if the sql queries are provided by the user then access this tool to execute that, and in case if query is modified then access this to execuet the query",
    )
]
...
prefix = """
You are a PotgrSQL expert. Given an input question, first create a syntactically correct PotgrSQL query to 
...
User: Filter the top 5 participants based on their scores in percentage for the batch TRED-DE-07092023.
In the given question batch TRED-DE-07092023 is the substring of the description attribute in the orders_dimension table.
...
message_history = SQLChatMessageHistory(
    session_id="new-session-2", connection_string="sqlite:///sqlite.db"
)
